...
class SantaBot:
    def __init__(self):
        ...
        self.gift_request_re = re.compile(request_regex_string, re.IGNORECASE)

        self.session = Session()

    def read_config(self):
        import os
        self.token = str(os.environ.get('TOKEN', 3))
    ...
    def main(self):
        ...
        handlers = [
            ...
            MessageHandler(Filters.reply & Filters.text, self.request)
        ]
    ...
    def status(self, bot, update):
        try:
            ...
            if not group_exists:
                ...
                print(self.session.query(Link).filter(Link.group_id == chat_id).join(Participant, Link.santa_id == Participant.id).all())

                currentLinks = self.session.query(Link).filter(Link.group_id == chat_id).join(Participant, Link.santa_id == Participant.id).all()
    ...
    def help(bot, update):
        message = "Command List: \n" \
                  ...
                  "/request \n" \
                  "Sent only in a private message to show and update your gift request. \n" \
                  "/join \n" \
    ...
    def request(self, bot, update):
        try:
            ...
            if original_user.id == self.bot_id and \
                    original_text == "What is your secret santa gift request? (Reply to this message to change it)":
                # request_match_object = self.gift_request_re.match(new_request)
                # if request_match_object is not None:
                # print(request_match_object)
                this_user = self.session.query(Participant).filter(
                    Participant.telegram_id == update.message.from_user.id).first()
                # print(this_user)
                # print(this_user.id)
                # request_filtered = request_match_object.group()
                # print(request_filtered)
                this_user.gift_request = new_request.strip()
                self.session.commit()
                message = "OK, I have added your secret santa gift request as: " + this_user.gift_request + "\n" \
                ...
                update.message.reply_text(message)
                # else:
                #     message = "This is not a valid request. An example is: 350 Fifth Ave. New York, NY 10118"
                #     update.message.reply_text(message)
                #     bot.send_message(chat_id=update.message.from_user.id,
                #                      text="What is your secret santa gift request? (Reply to this message to change it)",
    ...
    def join(self, bot, update):
        try:
            ...
            if this_participant.gift_request is None:
                message = "I need your secret santa gift request first! I am sending you a private message now."
                update.message.reply_text(message)
                message = "What is your secret santa gift request? (Reply to this message to change it)"
                bot.send_message(chat_id=this_participant.telegram_id, text=message, reply_markup=ForceReply())
                return
    ...
    def start_exchange(self, bot, update):
        try:
            ...
            for santa in group_participants_objects:
                ...
                message = "You got " + receiver.telegram_username + "! Their request is: " + receiver.gift_request
